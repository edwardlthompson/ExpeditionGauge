...
| Status | Owner | Task |
|--------|-------|------|
| 🔲 | [ADB] | M-003 Device validation: install **2.14.1** (`category.POI`) → `aa-refresh-host.ps1 -Clear` → Customize launcher → USB head unit; if still missing see Escalation in `docs/help/ANDROID_AUTO.md` (no more category swaps) |
