...
class FlyoverVideoExportWorker(
    appContext: Context,
    ...
    override suspend fun doWork(): Result {
        ...
        val clipDurationMs = inputData.getLong(KEY_CLIP_DURATION_MS, 30_000L)
        ...
        val media = database.sessionMediaDao().getBySession(sessionId)

        val settings = FlyoverVideoExportSettings(clipDurationMs = clipDurationMs)
        val outputDir = File(applicationContext.cacheDir, "exports").apply { mkdirs() }
        val outputFile = File(outputDir, "session_${sessionId}_flyover.mp4")
        ...
        val exportResult = exporter.export(
            ...
            onProgress = { percent -> setProgressAsync(workDataOfProgress(percent)) },
            frameDelayMs = { FlyoverThermalGuard.frameDelayMs(applicationContext) },
        )

        return exportResult.fold(
            onSuccess = {
                Result.success(
                    Data.Builder().putString(KEY_OUTPUT_PATH, outputFile.absolutePath).build(),
                )
            },
    ...
    companion object {
        ...
        const val WORK_NAME_PREFIX = "flyover_video_export_"
