...
object DriveHudLetterbox {
    fun toSquare(strip: Bitmap, darkBackground: Boolean = true, highContrast: Boolean = false): Bitmap {
        val size = strip.width.coerceAtLeast(1)
        val out = Bitmap.createBitmap(size, size, Bitmap.Config.ARGB_8888)
        val canvas = Canvas(out)
        val theme = DriveHudTheme.forDarkMode(darkBackground, highContrast = highContrast)
