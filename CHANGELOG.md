...
## [2.19.0] — Privacy backup, Relive, and offline maps (2026-08-29)
...
* Child CI Feature Gate uses `--stack android`; Validate Bootstrap `--quick` sets `BOOTSTRAP_QUICK` so missing optional linters skip without unsetting `GITHUB_ACTIONS`; product mode skips `init-project` upgrade simulation
...
* AA ROW Drive HUD **OBD DTC footer**: one-shot Mode 03 on connect; vendored OBDex CC0 catalog (`obdex_en.gz`); 5 s carousel; permanent band; COLUMN omits footer
...
* Store OBDex DTC titles as gzip asset (obdex_en.gz) to stay under 500 KB tracked-file hygiene budget
...
* Stationary pose auto-calibrate: after ~3s still in the mount, confirm dialog zeros pitch/roll/yaw (Settings toggle, default on)
* Soft magnetometer heading for body yaw when field is stable; P/R-only Zero when mag is distorted
* Whole-degree pitch/roll/yaw labels on phone inclinometer and Android Auto Attitude tile (graphics stay float)

### Changed

* Calibration offsets include yaw; Zero / Reset / AA Zero share one commit gate (manual Zero blocks auto for 2s)

### Fixed

* Pre-release Dependabot alert count uses `gh api --paginate` (no unsupported `page=` query)
* Windows Git Bash: MSYS `/c/...` roots no longer break bootstrap/readme/agent-progress Python paths
* `feature-gate --stack multi --strict` no longer fails when optional go/npm/cargo is missing
