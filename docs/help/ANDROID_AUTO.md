...
- **Wide** surface: native **3×1** Attitude | Telemetry | TPMS. Top: **Mute**, **Capture**, **Record/Stop**, parked-only **Level**. Tap the **left** attitude cube to cycle inclinometer styles → G-meter → 3D compass. A **permanent bottom DTC band** sits under the cubes (always reserved). When OBD has stored codes (Mode 03 on connect + gated ~30 s refresh, or debug sim), bold-red single-line text carousels each code (`n/N` + description, 5 s), **scaled to fit the row width** (shrink/expand; ellipsis only if still too long at minimum size), vertically centered in the band. Empty = blank band. Attitude taps ignore the footer.
