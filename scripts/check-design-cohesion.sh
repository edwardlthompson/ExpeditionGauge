...
if [ -f examples/android/app/src/main/java/dev/foss/goldenpath/ui/theme/Color.kt ]; then
  REQUIRED_OUTPUTS+=(
...
elif [ -d examples/android ]; then
  # Child product under examples/android — do not require template goldenpath paths.
  if ! find examples/android -path '*/ui/theme/Color.kt' | grep -q .; then
    fail "missing Android Color.kt theme tokens"
  fi
fi
