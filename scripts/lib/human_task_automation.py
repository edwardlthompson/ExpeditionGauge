...
def _parse_sprint_id(sprint: str, task: str) -> str:
    m = re.search(r"Sprint\s+(\d+[a-z]?)", sprint, re.I)
    if m:
        return m.group(1)
    m = re.search(r"Sprint\s+(\d+[a-z]?)", task, re.I)
    if m:
        return m.group(1)
    m = re.search(r"-Sprint\s+(\d+[a-z]?)", task, re.I)
...
def _parse_adb_scenario(task: str) -> str:
    m = re.search(r"`([a-z0-9][a-z0-9-]+)`", task)
    if m:
        return m.group(1)
    return ""


def automate_expedition_adb_smoke(root: Path, cfg: dict, task: str, sprint: str) -> AttemptResult:
    script = root / "scripts/expedition/adb-smoke.ps1"
    if not script.is_file():
        return AttemptResult(1, "adb-smoke", "scripts/expedition/adb-smoke.ps1 missing", True)
    sprint_id = _parse_sprint_id(sprint, task)
    scenario = _parse_adb_scenario(task)
    if not sprint_id or not scenario:
        return AttemptResult(1, "adb-smoke", "Could not parse sprint/scenario from task", True)
    if not adb_authorized(root):
        return AttemptResult(1, "adb-unavailable", "no_authorized_device", True)
    code, tail = run_cmd(
        root,
        ["pwsh", "-NoProfile", "-File", str(script), "-Sprint", sprint_id, "-Scenario", scenario],
    )
    if code == 0:
        return AttemptResult(0, "adb-smoke", f"Scenario {scenario} passed", False)
    return AttemptResult(1, "adb-smoke", tail or f"exit {code}", True)


def automate_expedition_auto_row(root: Path, cfg: dict, task: str, sprint: str) -> AttemptResult:
    sprint_id = _parse_sprint_id(sprint, task)
    for gate in re.findall(r"check-v2-[a-z-]+-gate\.sh", task):
        gate_path = root / "scripts" / gate
        if gate_path.is_file():
            code, tail = run_cmd(root, ["bash", str(gate_path)])
            if code != 0:
                return AttemptResult(1, gate, tail or f"exit {code}", True)
    for gate in re.findall(r"check-[a-z-]+-gate\.sh", task):
        if gate.startswith("check-v2-"):
            continue
        gate_path = root / "scripts" / gate
        if gate_path.is_file():
            code, tail = run_cmd(root, ["bash", str(gate_path)])
            if code != 0:
                return AttemptResult(1, gate, tail or f"exit {code}", True)
    if "sprint-signoff.ps1" in task and sprint_id:
        signoff = root / "scripts/expedition/sprint-signoff.ps1"
        if signoff.is_file():
            code, tail = run_cmd(
                root,
                ["pwsh", "-NoProfile", "-File", str(signoff), "-Sprint", sprint_id],
            )
            if code != 0:
                return AttemptResult(1, "sprint-signoff", tail or f"exit {code}", True)
    return AttemptResult(0, "expedition-auto", "AUTO gate scripts passed", False)
...
ADB_RULES: list[tuple[re.Pattern[str], str, object]] = [
    (re.compile(r"adb-smoke|`\w[\w-]+`", re.I), "adb", automate_expedition_adb_smoke),
    ...
    (re.compile(r"emulator|Android SDK|OnePlus|device", re.I), "adb", automate_android_sdk_smoke),
]

AUTO_RULES: list[tuple[re.Pattern[str], str, object]] = [
    (re.compile(r"check-v2-|check-.*-gate|sprint-signoff", re.I), "auto", automate_expedition_auto_row),
]
...
def attempt_row(root: Path, owner: str, task: str, sprint: str) -> AttemptResult:
    ...
    for pattern, _kind, handler in rules:
        if not pattern.search(task):
            continue
        if handler in (automate_approve_adr, automate_expedition_adb_smoke, automate_expedition_auto_row):
            return handler(root, cfg, task, sprint)  # type: ignore[operator]
